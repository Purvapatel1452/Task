...
import { fetchRecipientData, fetchUserExpenses } from '../redux/slices/recepientSlice';
...
const ChatMessageScreen = ({navigation}: any) => {
  ...
  const {recepientDatas,userExpense,loading:recepientLoading,error:recepientError}=useSelector((state)=>state.recepient)
    ...
    <Text style={{color:'black'}}>Chat</Text>
     ...
     { isExpense?
      ...
      {recepientLoading ? (
            ...
            userExpense.map((item, index) => <ExpenseBox key={index} item={item} />)
          )}
